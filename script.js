...
function checkCollision() {
    ...
    for (let i = 1; i < snake.length; i++) { // fixed typo from 'lenght'
...
function drawGrid() {
    ...
    for (let j = 0; j < canvas.height; j += GRID_SIZE) { // fixed typo from 'i < canvas.height' to 'j < canvas.height'
        context.beginPath();
        context.moveTo(0, j);
...
function gameOver() {
    gamePaused = true;
    gameOverScreen.style.display = 'block'; // fixed to 'block'
}
...
restartBtn.addEventListener('click', function() {
    ...
    gamePaused = false;
    initializeGame(); // fixed missing invocation
    update(); // fixed missing invocation
});
